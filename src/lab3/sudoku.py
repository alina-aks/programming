...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    True
    """
    variants = set(map(str, range(1,10)))
    used_row = set(get_row(grid, pos))
    used_col = set(get_col(grid,pos))
    used_block = set(get_block(grid,pos))
    used_placec = used_row | used_col | used_block
    variants -= used_placec
    return variants
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    """
    emp_variants = find_empty_positions(grid)
    if not emp_variants:
        return grid
    row, col = emp_variants
    pos_var = find_possible_values(grid, (row, col))
    for value in pos_var:
        grid[row][col] = value
        if solve(grid):
            return grid
        grid[row][col] = "."
